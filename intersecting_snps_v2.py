""" Adapted from WASP/find_interesecting_snps.py. Version 2 is much faster by counting ref and alt alleles in snptable object """
...
class DataFiles(object):
    ...
    def close(self):
        ...
        # filehandles = [self.initial_AI_pickle, self.fastq1,
...
def generate_reads2(reads,  pair_snp_read_pos, pair_snp_idx, ref_alleles, alt_alleles, genome_pos, snp_tab):
    ...
    Returns None when the original read pair has discordant alleles at shared SNPs. Also keeps records for reads overlapping ref and alt alleles by updating snp_tab """
...
def generate_reads_single(read_seq,  snp_read_pos, snp_idx, ref_alleles, alt_alleles, snp_tab):
    """ Generate a read  with opposite alleles to the ones observed.
     Also keeps records for reads overlapping ref and alt alleles by updating snp_tab """

    
    idx =  [x-1 for x in snp_read_pos]
    for i in range(len(idx)):
        if read_seq[idx[i]] == ref_alleles[i].decode("utf-8"):# index 0 is to unlist

            read_seq = read_seq[:idx[i]] + alt_alleles[i].decode("utf-8") + read_seq[idx[i]+1:]
            snp_tab.snp_na1[snp_idx[i]] +=1
            snp_tab.snp_na2[snp_idx[i]] +=1

        elif read_seq[idx[i]] == alt_alleles[i].decode("utf-8"):
            
            read_seq = read_seq[:idx[i]] + ref_alleles[i].decode("utf-8") + read_seq[idx[i]+1:]
            snp_tab.snp_na1[snp_idx[i]] +=1
            snp_tab.snp_na2[snp_idx[i]] +=1
...
def write_fastq(fastq_file, orig_read, new_seq, files):
    ...
    # 2 - the coordinates that it should map to (start (0-based) and end(1-based), see pysam fields)
    
    pos_str = "%d-%d" % (orig_read.pos+1, orig_read.reference_end)
        
    name = "%s.%s.%s" % (orig_read.qname, orig_read.reference_name, pos_str)
...
def filter_reads(files, base_qual=BASEQ_DEFAULT):
    ...
    for read in files.input_bam:
        ...
        if read.is_paired:
            ...
            #break
            process_single_read(read, read_stats, files, snp_tab, base_qual) 
...
def process_paired_read(read1, read2, read_stats, files,
    ...
    if len(pair_snp_idx[0]) !=0 or len(pair_snp_idx[1]) != 0:
        ...
        new_reads=generate_reads2(reads, pair_snp_read_pos, pair_snp_idx, ref_alleles, alt_alleles, genome_pos, snp_tab)
...
def process_single_read(read, read_stats, files, snp_tab,base_qual=BASEQ_DEFAULT):
    ...
    if len(snp_idx) > 0:
        ...
        if len(snp_idx) >0:
            ...
            read_seq = generate_reads_single(read.query_sequence,  snp_read_pos, snp_idx,
                                        ref_alleles, alt_alleles, snp_tab)
        
        #if len(read_seqs) < max_seqs:
            # write read to fastq file for remapping
            write_fastq(files.fastq_single, read, read_seq, files)          
